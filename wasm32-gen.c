...
static void w_layout(void)
{
    ...
    {
        int *sub_start = tcc_malloc(nsubs * sizeof(int));
        for (i = nsubs - 1; i >= 0; i--) {
        WSub *s = &subs[i];
        int last_has_edge = 0;
        sub_start[i] = blen + 1;   /* after the W_END below */
        body = wa_grow(body, &balloc, blen + 1, 1);
        ...
        /* remap patch offsets: the sub-block code is emitted in reverse
           order, so a patch inside sub-block s lands at
           sub_start[s] + (code_ofs - subs[s].start) */
        for (i = 0; i < wasm_cf->npatches; i++) {
            WasmPatch *p = &wasm_cf->patches[i];
            if (p->ofs >= w_body_start) {
                int s, ok = 0;
                for (s = 0; s < nsubs; s++) {
                    if (p->ofs >= subs[s].start && p->ofs < subs[s].end) {
                        p->ofs = sub_start[s] + (p->ofs - subs[s].start);
                        ok = 1;
                        break;
                    }
                }
                if (!ok)
                    tcc_error("wasm: patch outside sub-blocks");
            }
        }
        tcc_free(sub_start);
    }
